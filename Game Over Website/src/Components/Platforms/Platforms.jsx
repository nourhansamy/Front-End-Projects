...
export default function Platforms() {
  ...
  async function getPlatformGames(flag) {
    ...
    if (flag) {
      // Copy of games array
      let platformGamesCopy = [
        ...platformGames,
        ...data.slice(platformGames.length, platformGames.length + 20),
      ];
      setPlatformGames(platformGamesCopy);
    } else {
      setPlatformGames(data.slice(0, 20));
    }
  }
  function moreGames() {
    getPlatformGames(true);
  }
  useEffect(() => {
    // console.log("platformType changed!");
    getPlatformGames();
  }, [platformType]);
  return (
    <>
